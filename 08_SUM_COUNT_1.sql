...
SELECT
    SUM(CASE
			WHEN order_date = pref_delivery_date THEN 1 ELSE 0
		END) AS sum_case
FROM delivery; 

# [코드 8-2] SUM, COUNT 문제 1 최종 코드
SELECT
    ROUND((SUM(CASE WHEN order_date = pref_delivery_date
...
SELECT
    COUNT(CASE
			WHEN order_date = pref_delivery_date THEN 1
		END) AS count_case
FROM delivery; 

# [코드 8-4] SUM, COUNT 문제 1 최종 코드
SELECT
    ROUND((COUNT(CASE WHEN order_date = pref_delivery_date 
            THEN 1 END) / COUNT(*) * 100), 2) AS immediate_percentage
FROM delivery; 

# [코드 8-5] SUM, COUNT 문제 1 최종 코드
SELECT
	ROUND((SELECT COUNT(*) FROM delivery WHERE order_date = pref_delivery_date)
